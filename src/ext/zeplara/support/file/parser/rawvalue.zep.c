...
ZEPHIR_INIT_CLASS(Zeplara_Support_File_Parser_RawValue) {
	...
	ZEPHIR_REGISTER_CLASS(Zeplara\\Support\\File\\Parser, RawValue, zeplara, support_file_parser_rawvalue, zeplara_support_file_parser_rawvalue_method_entry, ZEND_ACC_FINAL_CLASS);

	/**
	 * @param string value
	 */
	zend_declare_property_null(zeplara_support_file_parser_rawvalue_ce, SL("value"), ZEND_ACC_PROTECTED);

	/**
	 * @var LinesIterator
	 */
	zend_declare_property_null(zeplara_support_file_parser_rawvalue_ce, SL("lines"), ZEND_ACC_PROTECTED);
...
PHP_METHOD(Zeplara_Support_File_Parser_RawValue, __construct) {
	...
	ZEPHIR_INIT_VAR(&_0);
	object_init_ex(&_0, zeplara_support_file_parser_linesiterator_ce);
	ZEPHIR_CALL_METHOD(NULL, &_0, "__construct", NULL, 173, &value);
