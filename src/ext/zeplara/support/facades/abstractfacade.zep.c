...
ZEPHIR_INIT_CLASS(Zeplara_Support_Facades_AbstractFacade) {
	...
	ZEPHIR_REGISTER_CLASS(Zeplara\\Support\\Facades, AbstractFacade, zeplara, support_facades_abstractfacade, zeplara_support_facades_abstractfacade_method_entry, ZEND_ACC_EXPLICIT_ABSTRACT_CLASS);

	zend_declare_property_null(zeplara_support_facades_abstractfacade_ce, SL("container"), ZEND_ACC_PROTECTED|ZEND_ACC_STATIC);
...
PHP_METHOD(Zeplara_Support_Facades_AbstractFacade, setContainer) {
	...
	zephir_update_static_property_ce(zeplara_support_facades_abstractfacade_ce, ZEND_STRL("container"), container);
...
PHP_METHOD(Zeplara_Support_Facades_AbstractFacade, getAccessor) {
	...
	ZEPHIR_CALL_METHOD(NULL, &_0, "__construct", NULL, 21, &_5);
	zephir_check_call_status();
	zephir_throw_exception_debug(&_0, "zeplara/Support/Facades/AbstractFacade.zep", 37);
	ZEPHIR_MM_RESTORE();
	return;
