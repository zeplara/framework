...
ZEPHIR_INIT_CLASS(Zeplara_File_Finder_DateFilter) {
	...
	ZEPHIR_REGISTER_CLASS(Zeplara\\File\\Finder, DateFilter, zeplara, file_finder_datefilter, zeplara_file_finder_datefilter_method_entry, ZEND_ACC_FINAL_CLASS);

	/**
	 * @var mixed
	 */
	zend_declare_property_null(zeplara_file_finder_datefilter_ce, SL("time"), ZEND_ACC_PRIVATE);
...
PHP_METHOD(Zeplara_File_Finder_DateFilter, __construct) {
	...
	zval *time = NULL, time_sub, *operator, operator_sub, _0$$3, _1$$5, _2$$5;
	...
	if (zephir_is_numeric(time) == 0) {
		ZEPHIR_CALL_FUNCTION(&_0$$3, "strtotime", NULL, 41, time);
		zephir_check_call_status();
		ZEPHIR_CPY_WRT(time, &_0$$3);
		if (ZEPHIR_IS_FALSE_IDENTICAL(time)) {
			ZEPHIR_THROW_EXCEPTION_DEBUG_STR(spl_ce_InvalidArgumentException, "The time is invalid type.", "zeplara/File/Finder/DateFilter.zep", 30);
			return;
		}
	}
...
PHP_METHOD(Zeplara_File_Finder_DateFilter, accept) {
	...
	do {
		...
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(spl_ce_InvalidArgumentException, "The operator is invalid.", "zeplara/File/Finder/DateFilter.zep", 79);
		return;
	} while(0);
