...
ZEPHIR_INIT_CLASS(Zeplara_File_Finder_FileExtensionFilter) {
	...
	ZEPHIR_REGISTER_CLASS(Zeplara\\File\\Finder, FileExtensionFilter, zeplara, file_finder_fileextensionfilter, zeplara_file_finder_fileextensionfilter_method_entry, ZEND_ACC_FINAL_CLASS);

	/**
	 * @var array
	 */
	zend_declare_property_null(zeplara_file_finder_fileextensionfilter_ce, SL("extensions"), ZEND_ACC_PRIVATE);

	/**
	 * @var int
	 */
	zend_declare_property_null(zeplara_file_finder_fileextensionfilter_ce, SL("state"), ZEND_ACC_PRIVATE);

	/**
	 * @var int
	 */
	zephir_declare_class_constant_long(zeplara_file_finder_fileextensionfilter_ce, SL("EXPECTED"), 1);
...
PHP_METHOD(Zeplara_File_Finder_FileExtensionFilter, __construct) {
	...
	if (zephir_fast_in_array(&_2, &_0) == 0) {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(spl_ce_InvalidArgumentException, "The state is invalid.", "zeplara/File/Finder/FileExtensionFilter.zep", 34);
		return;
	}
...
PHP_METHOD(Zeplara_File_Finder_FileExtensionFilter, accept) {
	...
	if (zephir_is_true(&_0)) {
		...
		if (ZEPHIR_IS_LONG_IDENTICAL(&_2$$3, 0)) {
			...
			ZEPHIR_CALL_FUNCTION(&_7$$3, "array_map", NULL, 67, &_6$$3, &_5$$3);
			zephir_check_call_status();
			...
			ZEPHIR_CALL_FUNCTION(&_11$$3, "array_map", NULL, 67, &_10$$3, &_9$$3);
			zephir_check_call_status();
