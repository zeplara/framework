...
ZEPHIR_INIT_CLASS(Zeplara_File_Finder_FileFinder) {
	...
	ZEPHIR_REGISTER_CLASS(Zeplara\\File\\Finder, FileFinder, zeplara, file_finder_filefinder, zeplara_file_finder_filefinder_method_entry, ZEND_ACC_FINAL_CLASS);

	/**
	 * @var string
	 */
	zend_declare_property_null(zeplara_file_finder_filefinder_ce, SL("path"), ZEND_ACC_PROTECTED);

	/**
	 * @var mixed
	 */
	zend_declare_property_null(zeplara_file_finder_filefinder_ce, SL("depth"), ZEND_ACC_PROTECTED);

	/**
	 * @var array
	 */
	zend_declare_property_null(zeplara_file_finder_filefinder_ce, SL("filters"), ZEND_ACC_PROTECTED);

	zend_class_implements(zeplara_file_finder_filefinder_ce, 1, zend_ce_aggregate);
	return SUCCESS;
...
PHP_METHOD(Zeplara_File_Finder_FileFinder, setDepth) {
	...
	zval *depth, depth_sub;
	zval *this_ptr = getThis();

	ZVAL_UNDEF(&depth_sub);

	zephir_fetch_params_without_memory_grow(1, 0, &depth);



	zephir_update_property_zval(this_ptr, ZEND_STRL("depth"), depth);
	RETURN_THISW();
...
PHP_METHOD(Zeplara_File_Finder_FileFinder, __construct) {
	...
	ZEPHIR_CALL_METHOD(&_0, this_ptr, "normalizepath", NULL, 68, path);
	zephir_check_call_status();
	zephir_update_property_zval(this_ptr, ZEND_STRL("path"), &_0);
	ZEPHIR_CALL_METHOD(NULL, this_ptr, "setdepth", NULL, 69, depth);
	zephir_check_call_status();
	ZEPHIR_CALL_METHOD(NULL, this_ptr, "setfilters", NULL, 70, &filters);
	zephir_check_call_status();
	ZEPHIR_MM_RESTORE();
...
PHP_METHOD(Zeplara_File_Finder_FileFinder, getIterator) {
	...
	ZEPHIR_CALL_METHOD(&_1, this_ptr, "loadpath", NULL, 71, &_2, &_3);
	zephir_check_call_status();
	ZEPHIR_CALL_METHOD(NULL, &_0, "__construct", NULL, 72, &_1);
	zephir_check_call_status();
	zephir_read_property(&_4, this_ptr, ZEND_STRL("filters"), PH_NOISY_CC | PH_READONLY);
	ZEPHIR_CALL_METHOD(NULL, return_value, "__construct", NULL, 73, &_0, &_4);
	zephir_check_call_status();
	RETURN_MM();
...
PHP_METHOD(Zeplara_File_Finder_FileFinder, __call) {
	...
	if ((zephir_method_exists(&factory, name)  == SUCCESS) == 1) {
		...
		zephir_check_call_status();
		ZEPHIR_CALL_METHOD(NULL, this_ptr, "addfilter", NULL, 74, &_1$$3);
		zephir_check_call_status();
	} else {
		...
		if (Z_TYPE_P(&filter$$4) == IS_NULL) {
			...
			zephir_check_call_status();
			zephir_throw_exception_debug(&_13$$7, "zeplara/File/Finder/FileFinder.zep", 81);
			ZEPHIR_MM_RESTORE();
			return;
			...
			zephir_check_call_status();
			zephir_throw_exception_debug(&_17$$8, "zeplara/File/Finder/FileFinder.zep", 83);
			ZEPHIR_MM_RESTORE();
			return;
		}
...
PHP_METHOD(Zeplara_File_Finder_FileFinder, instance) {
	...
	object_init_ex(return_value, zeplara_file_finder_filefinder_ce);
	ZEPHIR_CALL_METHOD(NULL, return_value, "__construct", NULL, 75, path, depth, &filters);
	zephir_check_call_status();
	RETURN_MM();
...
PHP_METHOD(Zeplara_File_Finder_FileFinder, loadPath) {
	...
	array_init(&entries);
	ZEPHIR_CALL_FUNCTION(&handle, "opendir", NULL, 76, path);
	zephir_check_call_status();
	while (1) {
		if (!(1)) {
			break;
		}
		ZEPHIR_CALL_FUNCTION(&entry, "readdir", &_3, 77, &handle);
		zephir_check_call_status();
		...
		ZEPHIR_CALL_METHOD(&entry, this_ptr, "normalizepath", &_8, 68, &_7$$4);
		zephir_check_call_status();
		ZEPHIR_INIT_NVAR(&_9$$4);
		object_init_ex(&_9$$4, zeplara_file_splfileinfo_ce);
		ZEPHIR_CALL_METHOD(NULL, &_9$$4, "__construct", &_10, 78, &entry);
		zephir_check_call_status();
		zephir_array_append(&entries, &_9$$4, PH_SEPARATE, "zeplara/File/Finder/FileFinder.zep", 163);
		ZEPHIR_CALL_FUNCTION(&_11$$4, "is_dir", &_12, 79, &entry);
		zephir_check_call_status();
		...
		if (_13$$4) {
			...
			ZEPHIR_CALL_METHOD(&_15$$7, this_ptr, "loadpath", &_17, 71, &entry, &_16$$7);
			zephir_check_call_status();
			zephir_fast_array_merge(&_14$$7, &entries, &_15$$7);
			ZEPHIR_CPY_WRT(&entries, &_14$$7);
		}
	}
	ZEPHIR_CALL_FUNCTION(NULL, "closedir", NULL, 80, &handle);
	zephir_check_call_status();
