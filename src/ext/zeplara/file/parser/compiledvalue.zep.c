...
ZEPHIR_INIT_CLASS(Zeplara_File_Parser_CompiledValue) {
	...
	ZEPHIR_REGISTER_CLASS(Zeplara\\File\\Parser, CompiledValue, zeplara, file_parser_compiledvalue, zeplara_file_parser_compiledvalue_method_entry, ZEND_ACC_FINAL_CLASS);

	/**
	 * @var mixed
	 */
	zend_declare_property_null(zeplara_file_parser_compiledvalue_ce, SL("value"), ZEND_ACC_PROTECTED);
...
PHP_METHOD(Zeplara_File_Parser_CompiledValue, __construct) {
	...
	ZEPHIR_INIT_VAR(&_0);
	object_init_ex(&_0, zeplara_file_parser_rawvalue_ce);
	ZEPHIR_CALL_METHOD(NULL, &_0, "__construct", NULL, 86, &rawValue);
