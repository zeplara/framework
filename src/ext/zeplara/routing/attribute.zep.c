...
ZEPHIR_INIT_CLASS(Zeplara_Routing_Attribute) {
	...
	ZEPHIR_REGISTER_CLASS(Zeplara\\Routing, Attribute, zeplara, routing_attribute, zeplara_routing_attribute_method_entry, ZEND_ACC_EXPLICIT_ABSTRACT_CLASS);

	/**
	 * @var array
	 */
	zend_declare_property_null(zeplara_routing_attribute_ce, SL("attributes"), ZEND_ACC_PROTECTED);

	/**
	 * @var array
	 */
	zend_declare_property_null(zeplara_routing_attribute_ce, SL("methodsCache"), ZEND_ACC_PRIVATE|ZEND_ACC_STATIC);

	zeplara_routing_attribute_ce->create_object = zephir_init_properties_Zeplara_Routing_Attribute;

	zend_class_implements(zeplara_routing_attribute_ce, 1, zeplara_contracts_routing_attribute_ce);
	return SUCCESS;
...
PHP_METHOD(Zeplara_Routing_Attribute, setAttributes) {
	...
	zephir_is_iterable(&attributes, 0, "zeplara/Routing/Attribute.zep", 39);
	if (Z_TYPE_P(&attributes) == IS_ARRAY) {
		ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(&attributes), _2, _3, _0)
		{
			...
			if (zephir_is_numeric(&key)) {
				ZEPHIR_THROW_EXCEPTION_DEBUG_STR(spl_ce_InvalidArgumentException, "The key of attributes should be string type.", "zeplara/Routing/Attribute.zep", 29);
				return;
			}
...
PHP_METHOD(Zeplara_Routing_Attribute, methodExists) {
	...
	zephir_read_static_property_ce(&_0, zeplara_routing_attribute_ce, SL("methodsCache"), PH_NOISY_CC | PH_READONLY);
	if (!(zephir_array_isset(&_0, &calledClass))) {
		...
		zephir_check_call_status();
		zephir_update_static_property_array_multi_ce(zeplara_routing_attribute_ce, SL("methodsCache"), &_2$$3, SL("z"), 1, &calledClass);
	}
	zephir_read_static_property_ce(&_3, zeplara_routing_attribute_ce, SL("methodsCache"), PH_NOISY_CC | PH_READONLY);
	zephir_array_fetch(&_4, &_3, &calledClass, PH_READONLY, "zeplara/Routing/Attribute.zep", 85);
...
PHP_METHOD(Zeplara_Routing_Attribute, throwBadMethodCallException) {
	...
	zephir_check_call_status();
	zephir_throw_exception_debug(&_0, "zeplara/Routing/Attribute.zep", 93);
	ZEPHIR_MM_RESTORE();
	return;
...
void zephir_init_static_properties_Zeplara_Routing_Attribute(TSRMLS_D) {
	...
	ZEPHIR_INIT_VAR(&_0);
	array_init(&_0);
	zephir_update_static_property_ce(zeplara_routing_attribute_ce, ZEND_STRL("methodsCache"), &_0);
	ZEPHIR_MM_RESTORE();
